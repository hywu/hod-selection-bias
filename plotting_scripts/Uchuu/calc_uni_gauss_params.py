...
x0_tophat = np.zeros(12)
x0_tophat[0:n_bins_myles] = x0_f_cl
x0_tophat[n_bins_myles:2*n_bins_myles] = x0_sigma_cl

#bounds for minimizer
bounds_tophat_lower = np.zeros(12) + 0.5
bounds_tophat_lower[n_bins_myles:] = 1
bounds_tophat_upper = np.ones(12)
bounds_tophat_upper[n_bins_myles:] = 13
bounds_tophat = Bounds(bounds_tophat_lower, bounds_tophat_upper)

chi_max = float((cat_name[7:])[:-4])

def norm(x, sigma):
    return np.exp(-0.5 * x**2 / sigma**2)/(sigma*np.sqrt(2*np.pi))
    
def lnlikelihood_single_uni_gauss(delta_chi, f_cl, sigma_cl):
    p_out = f_cl*norm(delta_chi, sigma_cl) + (1.0-f_cl)/(2*chi_max)
    sel = (p_out < 1e-20)
    p_out[sel] = 1e-20
    return np.log(p_out)
...
def calc_dbl_gauss_param(px_min, px_max, py_min, py_max, pz_min, pz_max):   
    ...
    def negative_lnlikelihood_total(theta):
        ...
        for i in range(n_bins_myles):
            ...
            tot += -np.sum(lnlikelihood_single_uni_gauss(dz_gal[sel], f_cl_l[i], sigma_cl_l[i]))
    ...
    res = minimize(negative_lnlikelihood_total, x0_tophat, bounds=bounds_tophat,\
