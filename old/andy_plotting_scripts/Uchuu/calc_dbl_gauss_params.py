...
x0_gauss = np.zeros(13)
x0_gauss[0:n_bins_myles] = x0_f_cl
x0_gauss[n_bins_myles:2*n_bins_myles] = x0_sigma_cl
x0_gauss[-1] = 85

bounds_lower_gauss = np.zeros(len(x0_gauss))
bounds_lower_gauss[n_bins_myles:] = 1
bounds_upper_gauss = np.ones(len(x0_gauss))
bounds_upper_gauss[n_bins_myles:2*n_bins_myles] = 13
bounds_upper_gauss[-1] = 1e3
bounds_gauss = Bounds(bounds_lower_gauss, bounds_upper_gauss)


def norm(x, sigma):
    return np.exp(-0.5 * x**2 / sigma**2)/(sigma*np.sqrt(2*np.pi))

def lnlikelihood_single_dbl_gauss(delta_chi, f_cl, sigma_cl, sigma_proj):
    p_out = f_cl*norm(delta_chi, sigma_cl) + (1-f_cl)*norm(delta_chi, sigma_proj)
    sel = (p_out < 1e-20)
    p_out[sel] = 1e-20
    return np.log(p_out)
...
def calc_dbl_gauss_param(px_min, px_max, py_min, py_max, pz_min, pz_max):   
    ...
    def negative_lnlikelihood_total(theta):
        ...
        sigma_proj_l = theta[-1]
        
        for i in range(n_bins_myles):
            sel = (binid == i) & (dz_gal != 0)
            
            tot += -np.sum(pmem[sel]*lnlikelihood_single_dbl_gauss(dz_gal[sel], f_cl_l[i], sigma_cl_l[i], \
                                                             sigma_proj_l))
    ...
    res = minimize(negative_lnlikelihood_total, x0_gauss, bounds=bounds_gauss,\
