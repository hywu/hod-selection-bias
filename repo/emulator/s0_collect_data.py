...
abun_or_lam = 'lam' #sys.argv[1] #'lam'#'abun'
emu_name = 'rad'
rich_name = 'q180'
zid = 3
phase = 0
...
for ibin in range(4):
    outfile_pca = open(f'{train_loc}/DS_{abun_or_lam}_bin_{ibin}_pca.dat', 'w')
    outfile_rad = open(f'{train_loc}/DS_{abun_or_lam}_bin_{ibin}_rad.dat', 'w')
    ...
    for imodel in range(n_models):
        ...
        lens_fname = f'{out_path}/obs_{rich_name}_desy1/DS_phys_noh_{abun_or_lam}_bin_{ibin}.dat'
